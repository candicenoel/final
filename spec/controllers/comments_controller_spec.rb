...
RSpec.describe CommentsController, type: :controller do
  describe "comments#create action" do
    it "should allow users to create comments on places" do
      ...
      post :create, params: { place_id: place.id, comment: { message: 'Facility info' } }

      expect(response).to redirect_to root_path
      expect(place.comments.length).to eq 1
      expect(place.comments.first.message).to eq 'Facility info'
    end
    ...
    it "should return http status code of not found if the place isn't found" do
      user = FactoryBot.create(:user)
      sign_in user
      post :create, params: { place_id: 'YOLOSWAG', comment: { message: 'Facility info' } }
